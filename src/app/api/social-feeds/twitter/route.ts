...
import fallbackData from '@/data/latest-tweet.json';
...
export async function GET() {
  ...
  if (feedUrl) {
    try {
        ...
        if (items.length > 0) {
          const posts: SocialPost[] = items
            .map((match) => {
              ...
              const rawImageUrl = mediaMatch?.[1] || undefined;
              ...
              return { platform: 'twitter' as const, text, url, date, imageUrl };
            })
      ...
      console.error('Twitter RSS fetch failed:', error);
    }
  }

  // Fallback
  const posts: SocialPost[] = fallbackData.text
    ? [{ platform: 'twitter', text: fallbackData.text, url: fallbackData.url, date: fallbackData.date }]
    : [];
