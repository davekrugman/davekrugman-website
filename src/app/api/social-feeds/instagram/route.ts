...
import fallbackData from '@/data/fallback-instagram.json';
...
export async function GET() {
  ...
  if (feedUrl) {
    try {
        ...
        if (items.length > 0) {
          const posts: SocialPost[] = items
            .map((match) => {
              ...
              return { platform: 'instagram' as const, text, url, date, imageUrl };
            })
      ...
      console.error('Instagram RSS fetch failed:', error);
    }
  }
